  ...
  return (
    ...
    >
          ...
          <div className="hidden lg:block relative" ref={dropdownRef}>
            {!loading && user && (
              <>
                ...
                {isProfileOpen && (
                  <div
                    className={`absolute right-0 mt-3 w-52 rounded-2xl border shadow-xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden ${isLight ? "bg-white border-slate-100" : "bg-[#12121f] border-white/10"}`}
                  >
                    <div className="p-1.5">
                      <Link
                        href="/dashboard"
                        onClick={() => setIsProfileOpen(false)}
                        className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] font-medium transition-colors ${isLight ? "text-slate-600 hover:bg-slate-100" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] font-medium text-red-500 hover:bg-red-500/10 transition-colors text-left"
                      >
            ...
            {!loading && !user && (
              <Link
                href="/auth/login"
                className="px-4 py-1.5 rounded-full text-[13px] font-semibold bg-indigo-600 text-white shadow-md active:scale-95"
              >
                Join
              </Link>
            )}
      ...
      {isMobileMenuOpen && (
        ...
        >
          {/* 1. Mobile Profile Header (Uppore) */}
          {!loading && user && (
            <div
            ...
            >
              ...
              {/* 2. Mobile Quick Actions (Logout, Dashboard, Profile) */}
